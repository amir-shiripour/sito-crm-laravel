...
@section('content')
    <div class="space-y-6">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-xl font-bold">روزهای خاص / استثناهای ارائه‌دهنده</h1>
                <p class="text-sm text-gray-500 mt-1">
                    ارائه‌دهنده: <span class="font-semibold">{{ $provider->name }}</span>
                    @if($provider->email)
            ...
            <div class="flex items-center gap-2 text-sm">
                <a href="{{ route('user.booking.providers.availability.edit', $provider) }}"
        ...
        {{-- فرم --}}
        <form method="POST"
              action="{{ route('user.booking.providers.exceptions.store', $provider) }}"
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div>
                    <label class="block text-xs mb-1">این روز برای این ارائه‌دهنده تعطیل باشد؟</label>
                    <select name="is_closed"
                            class="w-full border rounded p-2 text-sm"
                            x-model="isClosed">
                        <option value="0" @selected(old('is_closed')==='0')>خیر</option>
                        <option value="1" @selected(old('is_closed')==='1')>بله</option>
                    </select>
            ...
            <div class="mt-4 border-t pt-4" x-show="isClosed == 0">
                <div class="flex items-center justify-between mb-2">
                    <h2 class="text-sm font-semibold">ساعات کاری ویژه این روز</h2>
                    <button type="button"
            ...
            <div class="pt-4 flex items-center justify-between">
                <p class="text-[11px] text-gray-500">
                    اگر این روز تعطیل نباشد و همه فیلدها را خالی بگذارید، استثنا حذف و از برنامه عادی استفاده می‌شود.
                </p>
        ...
        <div class="bg-white rounded border p-4 space-y-3">
            <h2 class="text-sm font-semibold mb-2">استثناهای این ارائه‌دهنده</h2>
            ...
            <div class="border rounded overflow-hidden">
                <table class="min-w-full text-xs">
                    ...
                    <tr>
                        ...
                        <th class="p-2 text-right">ساعات کاری</th>
                    ...
                    @forelse($exceptions as $ex)
                        ...
                        <tr class="border-t">
                            ...
                            <td class="p-2">
                                ...
                                @else
                                    <span class="px-2 py-0.5 rounded bg-emerald-50 text-emerald-700 text-[11px]">باز</span>
                                @endif
                            </td>
                            <td class="p-2">
                                @if(count($windows))
                            ...
                            <td class="p-2">{{ $ex->override_capacity_per_day ?? '---' }}</td>
                            <td class="p-2">
                                <form method="POST"
                                      action="{{ route('user.booking.providers.exceptions.destroy', [$provider, $ex]) }}"
                                      onsubmit="return confirm('حذف این استثنا؟');">
                                    @csrf
                                    @method('DELETE')
    ...
    <script>
        function exceptionForm() {
            return {
                ...
                workWindows: [],
                breaks: [],
                addWindow() { this.workWindows.push({start_local: '', end_local: ''}); },
                removeWindow(i) { this.workWindows.splice(i, 1); },
                addBreak() { this.breaks.push({start_local: '', end_local: ''}); },
                removeBreak(i) { this.breaks.splice(i, 1); },
            };
        }
