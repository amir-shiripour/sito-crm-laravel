            ...
            <div x-show="open"
                ...
                <div class="flex items-center justify-between px-5 py-4 border-b border-gray-100 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-800/50">
                    <h3 class="text-base font-bold text-gray-900 dark:text-white flex items-center gap-2">
                        ...
                        ایجاد سریع {{ config('clients.labels.singular') }}
                    </h3>
                ...
                <div class="px-5 py-6 space-y-4">
                    @php $hasQuickFields = false; @endphp

                    @foreach(($schema['fields'] ?? []) as $i => $field)
                        @continue(empty($field['quick_create']))
                        @php
                            $fid = $field['id'] ?? "qf{$i}";
                            $hasQuickFields = true;
                        @endphp
                ...
                <div class="bg-gray-50 dark:bg-gray-900/50 px-5 py-4 flex flex-row-reverse gap-2">
                    <button type="button"
                            ...
                            @if($hasQuickFields) @click="open = false" @else disabled @endif
