...
@section('content')
    ...
    <div class="w-full mx-auto px-2 sm:px-4 lg:px-6 py-8" x-data="proformaForm({
        ...
        <form action="{{ route('admin.accounting.proformas.store') }}" method="POST"
                ...
                <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div>
                        ...
                        <p class="text-sm text-gray-500 dark:text-gray-400 mt-2 mr-14 max-w-2xl leading-relaxed">
                            اطلاعات پیش فاکتور جدید را در این فرم وارد کنید.
                        </p>
                    </div>
                </div>
                            ...
                            <select name="client_id" id="client_id" class="{{ $selectClass }}" required>
                                ...
                                @foreach($clients as $client)
                                    <option value="{{ $client->id }}"
                                            @if(old('client_id') == $client->id) selected @endif>{{ $client->full_name . ' (' . $client->username . ')' }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div>
                            ...
                            @else
                                <input type="text" name="proforma_number" id="proforma_number"
                                       value="{{ old('proforma_number') }}" class="{{ $inputClass }}" required>
