        ...
        <div class="p-6 sm:p-8 flex flex-col lg:flex-row lg:items-center justify-between gap-6">
            ...
            <div class="flex flex-wrap items-center gap-3 shrink-0">
                ...
                @if(!$document->sourceDocument && !$isDocCancelled)
                    <form action="{{ route('admin.accounting.documents.cancel', $document->id) }}" method="POST" onsubmit="return confirm('آیا از لغو این سند دستی و برگشت/اصلاح مبالغ آن به خزانه‌داری اطمینان دارید؟');" class="inline">
                        @csrf
                        <button type="submit"
                ...
                @elseif($isDocCancelled)
                    <span class="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-amber-50 text-amber-700 dark:bg-amber-500/10 dark:text-amber-400 border border-amber-200 dark:border-amber-500/20 font-bold text-sm">
                                ...
                                <td class="px-4 py-4 align-top text-center whitespace-nowrap">
                                    @if(!is_null($rowBalAfter))
                                        <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-xl bg-sky-50 dark:bg-sky-500/10 text-sky-800 dark:text-sky-300 border border-sky-100 dark:border-sky-500/20 font-bold text-xs">
                                            <span class="tabular-nums font-black">{{ $faNum(number_format(CurrencyService::convertForDisplay($rowBalAfter))) }}</span>
                ...
                <div
                    ...
                    <div class="w-full sm:w-96 space-y-3 text-base">
                        <div class="flex justify-between items-center text-emerald-600 dark:text-emerald-400">
                            ...
                            <span class="tabular-nums font-black text-lg">{{ $faNum(number_format($totalDebit)) }} <span
