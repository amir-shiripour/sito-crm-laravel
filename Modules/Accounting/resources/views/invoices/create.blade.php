...
@section('title', 'صدور فاکتور دستی')
...
@section('content')
    ...
    <div class="max-w-7xl mx-auto px-2 sm:px-4 lg:px-6 py-8" x-data="invoiceForm({
        ...
        roundingAmount: {{ $roundingAmount ?? 1000 }}
    })">
        <form action="{{ route('admin.accounting.invoices.store') }}" method="POST" @submit.prevent="submitForm($event.target)">
            @csrf
                ...
                <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div>
                        ...
                        <p class="text-sm text-gray-500 dark:text-gray-400 mt-2 mr-14 max-w-2xl leading-relaxed">
                            اطلاعات فاکتور  جدید را در این فرم وارد کنید.
                        </p>
                    </div>
                </div>
                    ...
                    <div class="p-6 grid grid-cols-1 md:grid-cols-3 gap-6">
                        ...
                        <div class="md:col-span-1">
                            ...
                            <select name="client_id" id="client_id" class="{{ $selectClass }}" required>
                                ...
                                @foreach($clients as $client)
                                    <option value="{{ $client->id }}" @if(old('client_id') == $client->id) selected @endif>{{ $client->full_name . ' (' . $client->username . ')' }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div>
                            ...
                            @if($numberingMode === 'auto')
                                <input type="text" name="invoice_number" id="invoice_number" value="{{ $nextInvoiceNumber }}" class="{{ $inputClass }} bg-gray-100 dark:bg-gray-800 cursor-not-allowed" readonly>
                                <p class="text-xs text-gray-500 mt-1">شماره به صورت خودکار تولید شده است.</p>
                            @else
                                <input type="text" name="invoice_number" id="invoice_number" value="{{ old('invoice_number') }}" class="{{ $inputClass }}" required>
                                <p class="text-xs text-gray-500 mt-1">حالت شماره‌گذاری دستی فعال است.</p>
                            @endif
                        </div>
                        ...
                        <div>
                            ...
                            <input type="text" id="issue_date" name="issue_date" data-jdp value="{{ old('issue_date', jdate()->format('Y/m/d')) }}" class="{{ $inputClass }} dir-ltr text-center" required>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="space-y-4 bg-gray-50 dark:bg-gray-900/50 p-5 rounded-2xl border border-gray-100 dark:border-gray-700">
                            ...
                            <div class="flex justify-between items-center border-b border-gray-200 dark:border-gray-700 pb-3">
                                <div class="flex flex-col">
                                    <label for="tax" class="text-sm font-medium text-gray-600 dark:text-gray-400">درصد مالیات :</label>
    ...
    @push('scripts')
        <script>
            function invoiceForm(config) {
                return {
                    formData: {
                        items: {!! json_encode(old('items', [['item_code' => '', 'description' => '', 'quantity' => 1, 'unit_type' => $units[0] ?? 'عدد', 'unit_price' => '', 'discount' => 0]])) !!},
                        notes: '{{ old('notes', '') }}',
                        tax: '{{ old('tax') }}' !== '' ? '{{ old('tax') }}' : config.defaultTaxRate,
                    },
