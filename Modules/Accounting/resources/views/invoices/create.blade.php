...
<div class="max-w-screen-2xl mx-auto px-4 sm:px-6 lg:px-8 py-8" x-data="invoiceForm({
    ...
    <form action="{{ route('admin.accounting.invoices.store') }}" method="POST">
        @csrf
        <div class="space-y-8">
            {{-- Header --}}
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <h1 class="text-2xl font-bold text-gray-900 dark:text-white">صدور فاکتور جدید</h1>
            </div>
            ...
            <div class="{{ $cardClass }}">
                <div class="p-6 grid grid-cols-1 md:grid-cols-4 gap-6">
                    <div>
                        ...
                        <input type="text" name="invoice_number" id="invoice_number" value="{{ old('invoice_number', $newInvoiceNumber) }}"
                    ...
                    <div>
                        ...
                        <select name="client_id" id="client_id" class="{{ $inputClass }}" required>
                            ...
                            @foreach($customers as $customer)
                                <option value="{{ $customer->id }}" @selected(old('client_id') == $customer->id)>{{ $customer->full_name ?: $customer->username }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                        <label for="issue_date" class="{{ $labelClass }}">تاریخ صدور <span class="text-red-500">*</span></label>
                        <input type="text" id="issue_date" name="issue_date" value="{{ old('issue_date') }}" data-jdp class="{{ $inputClass }} dir-ltr text-center" required>
                    </div>
                    <div>
                        <label for="due_date" class="{{ $labelClass }}">تاریخ سررسید <span class="text-red-500">*</span></label>
                        <input type="text" id="due_date" name="due_date" value="{{ old('due_date') }}" data-jdp class="{{ $inputClass }} dir-ltr text-center" required>
                    </div>
                </div>
            </div>
            ...
            <div class="{{ $cardClass }}">
                <div class="px-6 py-4 border-b border-gray-100 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-900/30">
                    <h3 class="text-lg font-bold text-gray-900 dark:text-white">ردیف‌های فاکتور</h3>
                </div>
                <div class="p-6">
                            ...
                            <tbody>
                                <template x-for="(item, index) in items" :key="index">
                                    <tr class="border-b border-gray-100 dark:border-gray-800">
                                        ...
                                        <td class="px-2 py-3 text-right text-sm font-semibold text-gray-700 dark:text-gray-300" x-text="formatCurrency(item.quantity * item.price) + ' ' + currency"></td>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                <div class="lg:col-span-2 {{ $cardClass }}">
                    <div class="p-6">
                        ...
                        <textarea name="notes" id="notes" rows="8" class="{{ $inputClass }}">{{ old('notes') }}</textarea>
                    </div>
                </div>
                <div class="{{ $cardClass }}">
                    <div class="p-6 space-y-4">
                        ...
                        <div class="flex justify-between items-center">
                            ...
                            <div class="flex items-center gap-2">
                                <input type="number" name="discount" id="discount" x-model.number="discount" @input="calculateTotals" class="w-32 {{ $inputClass }} text-left dir-ltr !mt-0" min="0">
                                <span class="text-sm text-gray-500" x-text="currency"></span>
                            </div>
                        </div>
                        <div class="flex justify-between items-center">
                            <label for="tax" class="text-sm font-medium text-gray-600 dark:text-gray-400">مالیات (<span x-text="tax_percentage"></span>%):</label>
                            <span class="text-base font-bold text-gray-800 dark:text-gray-100" x-text="formatCurrency(tax_amount) + ' ' + currency"></span>
                        </div>
    ...
    function invoiceForm(data) {
        return {
            ...
            calculateTotals() {
                ...
                let taxableAmount = this.subtotal - (parseFloat(this.discount) || 0);
