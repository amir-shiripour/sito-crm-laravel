...
@section('content')
    {{-- کامپوننت ساده برای سلکت سرچ‌دار تک‌انتخابی --}}
    <script>
        function singleSelect(config) {
            return {
                ...
                init() {
                    // مقدار اولیه را در input مخفی سینک می‌کنیم
    ...
    @php
        ...
        $initialRelatedId = old('related_id', $relatedClient->id ?? '');
    @endphp
    ...
    <div class="w-full mx-auto px-4 py-8">
        ...
        <div class="flex items-center justify-between mb-6">
            <div>
                ...
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                    یک پیگیری برای خودتان یا همکاران‌تان ثبت کنید.
                </p>
            </div>
        ...
        <form method="POST" action="{{ route('user.followups.store') }}" class="space-y-6">
            @csrf
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <textarea name="description" rows="3"
                                  class="w-full rounded-xl border-gray-300 bg-white px-4 py-2.5 text-sm transition-shadow focus:border-amber-500 focus:ring-1 focus:ring-amber-500 dark:bg-gray-900 dark:border-gray-600 dark:text:white dark:focus:border-amber-500"
                                  placeholder="شرح مختصری از این پیگیری بنویسید...">{{ old('description') }}</textarea>
                    </div>
                        ...
                        <div>
                            ...
                            <select name="status"
                                ...
                                @foreach($statuses as $value => $label)
                                    <option value="{{ $value }}" @selected(old('status', \Modules\Tasks\Entities\Task::STATUS_TODO) === $value)>
                                        {{ $label }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <select name="priority"
                                ...
                                @foreach($priorities as $value => $label)
                                    <option value="{{ $value }}" @selected(old('priority', \Modules\Tasks\Entities\Task::PRIORITY_MEDIUM) === $value)>
                                        {{ $label }}
                                    </option>
                                @endforeach
                            </select>
            ...
            <div class="bg-gray-50/80 dark:bg-gray-800/50 rounded-2xl border border-gray-200 dark:border-gray-700 p-6 space-y-4">
                ...
                @if(!empty($canAssign) && $canAssign)
                    {{-- سلکتور سرچ‌دار تک‌انتخابی برای مسئول --}}
                    <div class="space-y-2">
                        <label class="block text-sm font-medium mb-1.5 text-gray-700 dark:text-gray-300">
                            انتخاب مسئول
                        </label>
                        ...
                        <div
                            x-data="singleSelect({
                                ...
                                initialValue: '{{ old('assignee_id', $currentUser->id ?? '') }}',
                        ...
                        <p class="mt-1 text-[11px] text-gray-500 dark:text-gray-400">
                            فقط نقش‌هایی که مجوز مدیریت پیگیری‌ها را دارند، می‌توانند مسئول را تغییر دهند.
                        </p>
                    </div>
                @else
                    {{-- بدون مجوز: مسئول خود کاربر فعلی --}}
                    @if($currentUser)
                        <div
            ...
            <div class="bg-gray-50/80 dark:bg-gray-800/50 rounded-2xl border border-gray-200 dark:border-gray-700 p-6 space-y-4">
                ...
                <div class="space-y-2">
                    ...
                    <p class="mt-1 text-[11px] text-gray-500 dark:text-gray-400">
                        ...
                        انتخاب {{ config('clients.labels.singular', 'مشتری') }} اختیاری است.
                    </p>
                </div>
            </div>
