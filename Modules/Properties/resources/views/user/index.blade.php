...
@section('content')
    <div class="max-w-7xl mx-auto px-4 py-8 space-y-6" x-data="propertyList()">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white dark:bg-gray-800 p-5 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm">
            <div>
                <h1 class="text-xl font-bold text-gray-900 dark:text-white flex items-center gap-2">
                    ...
                    {{ $isTrash ? 'سطل زباله املاک' : 'لیست املاک' }}
                </h1>
                <p class="text-sm text-gray-500 dark:text-gray-400 mt-1 mr-10">
                    {{ $isTrash ? 'مشاهده و مدیریت املاک حذف شده' : 'مدیریت و مشاهده وضعیت املاک ثبت شده' }}
                </p>
            </div>
        ...
        {{-- فیلترها (فقط در حالت عادی نمایش داده شود) --}}
        @if(!$isTrash)
                                ...
                                <div class="flex items-center justify-end gap-2 opacity-80 group-hover:opacity-100 transition-opacity">
                                    @if($isTrash)
                                        ...
                                        @if($canDelete)
                                            ...
                                            <form action="{{ route('user.properties.force-delete', $property->id) }}" method="POST"
                                                  onsubmit="return confirm('آیا از حذف کامل و غیرقابل بازگشت این ملک اطمینان دارید؟');"
                                                  class="inline-block">
                                                @csrf @method('DELETE')
                                                <button type="submit"
                                    ...
                                    @else
                                        ...
                                        @if($canDelete)
                                            <form action="{{ route('user.properties.destroy', $property) }}" method="POST"
                                                  onsubmit="return confirm('آیا از انتقال این ملک به سطل زباله اطمینان دارید؟');"
                                                  class="inline-block">
                                                @csrf @method('DELETE')
                                                <button type="submit"
                                                        class="p-2 rounded-lg text-red-600 bg-red-50 hover:bg-red-100 dark:text-red-400 dark:bg-red-900/20 dark:hover:bg-red-900/40 transition-colors"
                            ...
                            <td colspan="{{ $canManageProperties ? '8' : '6' }}" class="py-12 text-center">
                                <div class="flex flex-col items-center justify-center text-gray-400 dark:text-gray-500">
                                    ...
                                    <p class="text-base font-medium text-gray-900 dark:text-white">
                                        {{ $isTrash ? 'سطل زباله خالی است' : 'هیچ ملکی یافت نشد' }}
                                    </p>
                                    @if(!$isTrash)
