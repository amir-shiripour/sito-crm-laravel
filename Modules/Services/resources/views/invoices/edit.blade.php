...
@php
    ...
    $existingItems = $invoice->items->map(function ($item) use ($services, $defaultTaxRate) {
        ...
        return [
            ...
            'custom_service_name' => $service ? $service->name : ($item->description ?? ''),
...
@section('content')
    <div class="max-w-screen-2xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8" x-data="invoiceCreator()">
        ...
        <form id="invoiceForm" @submit="onSubmitCheck($event)"
              action="{{ route('services.invoices.update', $invoice) }}"
                                                            ...
                                                            <div class="flex justify-between items-center mb-2">
                                                                ...
                                                                <div class="flex items-center gap-2 shrink-0">
                                                                    <template
                                                                        ...
                                                                        <span
                                                                            class="flex items-center gap-1 text-sm font-black text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-500/10 px-2.5 py-1 rounded-md border border-indigo-200 dark:border-indigo-500/20 shadow-sm">
                                                                            +<span
                                                                                x-text="formatMoney(getCustomFieldPrice(item, field))"></span>
                                                                            <span
                                                                                class="text-[10px] font-bold text-indigo-500 dark:text-indigo-400/80">{{ $currencyLabel }}</span>
                                                                        </span>
                                                                    </template>
                                ...
                                <span class="flex items-center gap-2 shrink-0 flex-wrap">
                                    ...
                                    <span class="relative inline-flex items-center gap-1">
                                        <input type="text"
                                               ...
                                               dir="ltr" placeholder="۰">
                                        <button type="button" @click="toggleExtraDiscountType()"
            ...
            <div class="{{ $cardClass }} relative overflow-hidden">
                <div
                    class="absolute inset-0 bg-linear-to-br from-emerald-50/50 via-transparent to-transparent dark:from-emerald-500/5 pointer-events-none">
                </div>
                <div
        ...
        <script>
            document.addEventListener('alpine:init', () => {
                Alpine.data('invoiceCreator', () => ({
                    issueDate: @json(old('issue_date', $invoice->issue_date ? $invoice->issue_date->format('Y/m/d') : '')),
                    dueDate: @json(old('due_date', $invoice->due_date ? $invoice->due_date->format('Y/m/d') : '')),
                    ...
                    init() {
                        if (this.selectedCustomer) {
                            const customer = this.customersList.find(c => c.id == this.selectedCustomer);
                            if (customer) {
                    ...
                    onSubmitCheck(e) {
                        ...
                        const numberInput = @json($isProforma) ? form.querySelector('input[name="proforma_invoice_number"]') : form.querySelector('input[name="invoice_number"]');
