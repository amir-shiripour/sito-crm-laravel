...
@section('content')
    ...
    <div class="max-w-5xl mx-auto space-y-8 pb-10">
        ...
        {{-- Alerts --}}
        @if(session('success'))
            <div class="p-4 bg-emerald-50 border border-emerald-200 text-emerald-800 rounded-xl flex items-center gap-3 dark:bg-emerald-900/20 dark:border-emerald-800 dark:text-emerald-300">
        ...
        @if($errors->any())
            <div class="p-4 bg-red-50 border border-red-200 text-red-800 rounded-xl dark:bg-red-900/20 dark:border-red-800 dark:text-red-300">
        ...
        <div class="space-y-6">
            ...
            {{-- Add New Stage Form --}}
            <div class="bg-indigo-50 dark:bg-indigo-900/10 border border-indigo-100 dark:border-indigo-800 rounded-xl p-5">
                <h3 class="text-sm font-semibold text-indigo-900 dark:text-indigo-300 mb-3">افزودن مرحله جدید</h3>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-12 gap-4">
                        <div class="md:col-span-5">
                            <input type="text" name="name" placeholder="نام مرحله (مثلاً: ارسال پیامک تایید)" required
                                   class="block w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm dark:bg-gray-800 dark:border-gray-600 dark:text-white">
                        </div>
                        ...
                        <div class="md:col-span-2 text-left">
                            <button type="submit" class="w-full inline-flex justify-center items-center px-4 py-2 text-sm font-medium text-white bg-indigo-600 border border-transparent rounded-lg shadow-sm hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                                افزودن
                            </button>
                        </div>
                    </div>
                    <textarea name="description" rows="1" placeholder="توضیحات اختیاری..."
                              class="block w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm dark:bg-gray-800 dark:border-gray-600 dark:text-white"></textarea>
                </form>
            </div>
            ...
            <div class="space-y-6">
                @forelse($workflow->stages as $stage)
                    <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl shadow-sm overflow-hidden transition hover:shadow-md">
                        ...
                        <div class="p-5 space-y-6">
                            {{-- Edit Stage Form --}}
                            <details class="group">
                                <summary class="flex items-center gap-2 text-sm font-medium text-gray-500 dark:text-gray-400 cursor-pointer hover:text-indigo-600 dark:hover:text-indigo-400 transition select-none">
                                    <svg class="w-4 h-4 transition group-open:rotate-90" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path></svg>
                                    ویرایش مشخصات مرحله
                                </summary>
                                <div class="mt-4 pl-4 border-l-2 border-gray-100 dark:border-gray-700">
                                    <form method="post" action="{{ route('user.workflows.stages.update', [$workflow, $stage]) }}" class="space-y-4">
                                        @csrf
                                        @method('patch')
                                        <div class="grid grid-cols-1 md:grid-cols-12 gap-4">
                                            <div class="md:col-span-5">
                                                <label class="block text-xs font-medium text-gray-500 mb-1">نام مرحله</label>
                                                <input type="text" name="name" value="{{ $stage->name }}" required
                                                       class="block w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm dark:bg-gray-800 dark:border-gray-600 dark:text-white">
                                            </div>
                                            <div class="md:col-span-2">
                                                <label class="block text-xs font-medium text-gray-500 mb-1">ترتیب</label>
                                                <input type="number" name="sort_order" value="{{ $stage->sort_order }}" min="0"
                                                       class="block w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm dark:bg-gray-800 dark:border-gray-600 dark:text-white">
                                            </div>
                                            <div class="md:col-span-3 flex items-center gap-4 pt-6">
                                                <label class="inline-flex items-center gap-2 text-sm text-gray-700 dark:text-gray-300 cursor-pointer">
                                                    <input type="checkbox" name="is_initial" value="1" @checked($stage->is_initial) class="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500 dark:bg-gray-700 dark:border-gray-600">
                                                    <span>شروع</span>
                                                </label>
                                                <label class="inline-flex items-center gap-2 text-sm text-gray-700 dark:text-gray-300 cursor-pointer">
                                                    <input type="checkbox" name="is_final" value="1" @checked($stage->is_final) class="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500 dark:bg-gray-700 dark:border-gray-600">
                                                    <span>پایان</span>
                                                </label>
                                            </div>
                                            <div class="md:col-span-2 pt-5 text-left">
                                                <button type="submit" class="text-sm text-indigo-600 hover:text-indigo-800 font-medium dark:text-indigo-400 dark:hover:text-indigo-300">
                                                    ذخیره تغییرات
                                                </button>
                                            </div>
                                        </div>
                                        <div>
                                            <label class="block text-xs font-medium text-gray-500 mb-1">توضیحات</label>
                                            <textarea name="description" rows="2"
                                                      class="block w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm dark:bg-gray-800 dark:border-gray-600 dark:text-white">{{ $stage->description }}</textarea>
                                        </div>
                                    </form>
                                </div>
                            </details>
                ...
                @empty
                    <div class="text-center py-12 bg-white dark:bg-gray-800 border border-dashed border-gray-300 dark:border-gray-700 rounded-xl">
