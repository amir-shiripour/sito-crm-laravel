...
<form method="post" action="{{ $formAction }}"
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
            <div>
                ...
                <textarea name="config[message]" rows="2"
                          :disabled="actionType !== '{{ \Modules\Workflows\Entities\WorkflowAction::TYPE_SEND_SMS }}'"
                          class="w-full rounded-lg border-gray-200 text-sm px-2 py-1.5 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-100" placeholder="سلام {client_name} عزیز ...">{{ $cfg['message'] ?? '' }}</textarea>
            </div>
            <div>
                    ...
                    @foreach($tokenOptions as $tokenKey => $tokenLabel)
                        <label class="inline-flex items-center gap-2 text-xs text-gray-700 dark:text-gray-200">
                            <input type="checkbox" name="config[params][]" value="{{ $tokenKey }}"
                                   :disabled="actionType !== '{{ \Modules\Workflows\Entities\WorkflowAction::TYPE_SEND_SMS }}'"
                                   @checked(in_array($tokenKey, $cfg['params'] ?? [])) class="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500">
                            {{ $tokenLabel }}
                        </label>
                    @endforeach
                </div>
    ...
    <div class="space-y-3" x-show="actionType === '{{ \Modules\Workflows\Entities\WorkflowAction::TYPE_CREATE_TASK }}' || actionType === '{{ \Modules\Workflows\Entities\WorkflowAction::TYPE_CREATE_FOLLOWUP }}'">
        ...
        <div>
            ...
            <textarea name="config[description]" rows="2"
                      :disabled="! (actionType === '{{ \Modules\Workflows\Entities\WorkflowAction::TYPE_CREATE_TASK }}' || actionType === '{{ \Modules\Workflows\Entities\WorkflowAction::TYPE_CREATE_FOLLOWUP }}')"
                      class="w-full rounded-lg border-gray-200 text-sm px-2 py-1.5 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-100">{{ $cfg['description'] ?? '' }}</textarea>
        </div>
    </div>
    ...
    <div class="space-y-3" x-show="actionType === '{{ \Modules\Workflows\Entities\WorkflowAction::TYPE_CREATE_REMINDER }}'">
        ...
        <div>
            ...
            <textarea name="config[message]" rows="2"
                      :disabled="actionType !== '{{ \Modules\Workflows\Entities\WorkflowAction::TYPE_CREATE_REMINDER }}'"
                      class="w-full rounded-lg border-gray-200 text-sm px-2 py-1.5 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-100">{{ $cfg['message'] ?? '' }}</textarea>
        </div>
    </div>
