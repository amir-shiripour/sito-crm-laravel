...
@section('title', 'ویرایش الگوی ساختار «' . $template->title . '»')
...
@php
    $initialData = $initialData ?? [
        ...
        'structure' => $template->structure ?? ['phases' => [], 'unphased_tasks' => []],
    ];
@endphp
    ...
    <div class="p-4 sm:p-6 lg:p-8 space-y-8 max-w-7xl mx-auto"
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 bg-white dark:bg-gray-800/90 rounded-3xl p-6 border border-gray-100 dark:border-gray-700/60 shadow-xs backdrop-blur-xl">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <div class="flex items-center gap-2 text-xs font-bold text-gray-400 mb-1">
                        ...
                        <span>/</span>
                        <span class="text-gray-600 dark:text-gray-300 truncate max-w-xs">{{ $template->title }}</span>
                        <span>/</span>
                        <span class="text-indigo-600 dark:text-indigo-400">ویرایش</span>
                    </div>
                    <h1 class="text-xl sm:text-2xl font-black text-gray-900 dark:text-white tracking-tight flex items-center gap-3">
                        <span class="flex items-center justify-center w-10 h-10 rounded-2xl bg-gradient-to-br from-amber-500 via-amber-600 to-indigo-600 text-white shadow-md shadow-amber-500/25 shrink-0">
                            <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                            </svg>
                        </span>
                        <span>ویرایش الگوی ساختار پروژه</span>
                    </h1>
                </div>
            </div>
            ...
            <div class="flex items-center gap-2.5 flex-wrap">
                {{-- Export JSON Button --}}
                <a href="{{ route('projects.templates.export', $template) }}"
                   class="px-4 py-2.5 rounded-2xl bg-indigo-50 hover:bg-indigo-100 dark:bg-indigo-900/30 dark:hover:bg-indigo-900/50 text-indigo-700 dark:text-indigo-300 text-xs font-bold border border-indigo-200 dark:border-indigo-800/40 transition-all flex items-center gap-1.5 shadow-2xs">
                ...
                <a href="{{ route('projects.templates.show', $template) }}"
                   class="px-4 py-2.5 rounded-2xl bg-gray-100 hover:bg-gray-200 dark:bg-gray-700 dark:hover:bg-gray-600 text-gray-700 dark:text-gray-300 text-xs font-bold transition-all flex items-center gap-1.5 shadow-2xs">
        ...
        {{-- Main Form --}}
        <form id="template-edit-form" method="POST" action="{{ route('projects.templates.update', $template) }}" @submit="prepareSubmit($event)" class="space-y-8">
            @csrf
            @method('PUT')
            <input type="hidden" name="structure" :value="JSON.stringify(structure)">

            {{-- ======================================================== --}}
            {{-- PARTITION 1: مشخصات کلی و شناسنامه الگو                  --}}
            {{-- ======================================================== --}}
                ...
                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-4 border-b border-gray-100 dark:border-gray-700/60">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-gray-400 mt-0.5">نام، دسته‌بندی و توضیحات کلی این الگو را ویرایش کنید.</p>
                        </div>
                    </div>
            ...
            {{-- ======================================================== --}}
            {{-- PARTITION 2: ابزار بارگذاری و شناسایی الگو از فایل JSON   --}}
            {{-- ======================================================== --}}
            <div class="bg-indigo-50/40 dark:bg-gray-800/60 rounded-3xl border-2 border-dashed border-indigo-200/80 dark:border-gray-700/80 shadow-xs p-6 sm:p-7 space-y-5">
                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                    <div class="flex items-center gap-3">
                        <span class="w-9 h-9 rounded-xl bg-purple-100 dark:bg-purple-900/40 text-purple-700 dark:text-purple-300 flex items-center justify-center font-black text-sm">
                            ۲
                        </span>
                        <div>
                            <h2 class="text-base font-black text-gray-900 dark:text-white flex items-center gap-2">
                            ...
                            <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
                                می‌توانید فایل JSON جدیدی بارگذاری کنید تا ساختار فعلی جایگزین یا تکمیل شود.
                            </p>
                        </div>
                    </div>
                ...
                <template x-if="jsonFeedback">
                    <div class="p-4 rounded-2xl flex items-start gap-3 transition-all"
                        ...
                        <span class="p-1 rounded-lg shrink-0 mt-0.5"
                            ...
                            <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
                                <path stroke-linecap="round" stroke-linejoin="round" :d="jsonFeedback.success ? 'M5 13l4 4L19 7' : 'M6 18L18 6M6 6l12 12'"/>
                            </svg>
                        </span>
                        <div class="flex-1 text-xs font-bold leading-relaxed" x-text="jsonFeedback.message"></div>
                        <button type="button" @click="jsonFeedback = null" class="text-gray-400 hover:text-gray-600 text-xs">✕</button>
                    </div>
                </template>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-12 gap-4 items-center">
                    ...
                    <div class="lg:col-span-8">
                        <label class="relative flex flex-col items-center justify-center p-5 rounded-2xl border-2 border-dashed transition-all cursor-pointer group bg-white/70 dark:bg-gray-900/50"
                               :class="isDraggingJson ? 'border-indigo-500 bg-indigo-50/50 dark:bg-indigo-950/30 ring-2 ring-indigo-500/20' : 'border-indigo-200/80 dark:border-gray-700 hover:border-indigo-400 dark:hover:border-indigo-600'"
                            ...
                            <input type="file" accept=".json,application/json" @change="handleJsonFileInput($event)" class="sr-only">
                            <div class="flex items-center gap-3 text-center sm:text-right flex-col sm:flex-row">
                                <div class="w-12 h-12 rounded-2xl bg-indigo-100 dark:bg-indigo-900/40 text-indigo-600 dark:text-indigo-400 flex items-center justify-center shrink-0 group-hover:scale-105 transition-transform">
                                ...
                                <div class="space-y-0.5">
                                    ...
                                    <p class="text-[11px] text-gray-400 dark:text-gray-400">
                                        سیستم ساختارهای تو در تو (فاز، گروه، کار) را بی‌درنگ شناسایی و درون فرم بارگذاری می‌کند.
                                    </p>
                                </div>
                            </div>
                        </label>
                    ...
                    <div class="lg:col-span-4 bg-white/70 dark:bg-gray-900/40 p-3.5 rounded-2xl border border-gray-200/70 dark:border-gray-700/60 space-y-2">
                        <label class="block text-xs font-bold text-gray-700 dark:text-gray-300">
            ...
            {{-- ======================================================== --}}
            {{-- PARTITION 3: ساختار فازها، گروه‌ها و کارها               --}}
            {{-- ======================================================== --}}
            <div class="space-y-6">
                {{-- Section Toolbar --}}
                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white dark:bg-gray-800/90 rounded-3xl p-5 border border-gray-100 dark:border-gray-700/60 shadow-xs">
                    <div class="flex items-center gap-3">
                        <span class="w-9 h-9 rounded-xl bg-indigo-50 dark:bg-indigo-900/40 text-indigo-600 dark:text-indigo-400 flex items-center justify-center font-black text-sm">
                            ۳
                        </span>
                        <div>
                            ...
                            <p class="text-xs text-gray-400 mt-0.5">
                                سلسله‌مراتب فازهای پروژه، گروه‌های کاری و چک‌لیست‌های مربوطه را ویرایش کنید.
                            </p>
                        </div>
                    </div>
                    ...
                    <div class="flex items-center gap-2 flex-wrap">
                        ...
                        <button type="button" @click="toggleSelectAll()"
                            ...
                            <span x-text="totalSelectedCount > 0 ? 'لغو انتخاب همه' : 'انتخاب همه فازها و گروه‌ها'"></span>
                        </button>
                ...
                {{-- Empty State --}}
                <template x-if="(!structure.phases || structure.phases.length === 0) && (!structure.unphased_tasks || structure.unphased_tasks.length === 0)">
                    <div class="bg-white dark:bg-gray-800/80 rounded-3xl p-12 text-center border-2 border-dashed border-gray-200 dark:border-gray-700 space-y-4">
                        <div class="w-16 h-16 rounded-2xl bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400 flex items-center justify-center mx-auto shadow-xs">
                        ...
                        <h3 class="text-base font-bold text-gray-900 dark:text-white">هنوز فاز یا گروهی به این الگو اضافه نشده است</h3>
                        <p class="text-xs text-gray-400 max-w-md mx-auto">
                            می‌توانید یک فایل JSON آپلود کنید تا خودکار پر شود، یا با کلیک بر روی دکمه‌های زیر فازها و گروه‌های جدید را دستی بسازید.
                        </p>
                ...
                <div class="space-y-6">
                    <template x-for="(phase, pIdx) in structure.phases" :key="'phase-' + pIdx">
                        <div class="bg-white dark:bg-gray-800/90 rounded-3xl border shadow-xs overflow-hidden transition-all duration-200"
                            ...
                            {{-- Phase Header Bar --}}
                            <div class="p-4 sm:p-5 bg-gray-50/90 dark:bg-gray-900/50 border-b border-gray-200/80 dark:border-gray-700/60 flex items-center justify-between gap-4 select-none flex-wrap">

                                <div class="flex items-center gap-3 min-w-0 flex-1">
                                    {{-- Checkbox with Clickable Label for Name/Row --}}
                                    <label class="flex items-center gap-2 cursor-pointer p-1.5 rounded-xl hover:bg-indigo-50 dark:hover:bg-indigo-900/30 transition-colors shrink-0"
                                        ...
                                        <span class="text-xs font-bold text-gray-600 dark:text-gray-300 select-none hidden md:inline">انتخاب</span>
                                    </label>

                                    {{-- Collapse / Expand Chevron Icon --}}
                                    <button type="button" @click="togglePhase(pIdx)"
                                            class="p-2 rounded-xl bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-gray-500 dark:text-gray-400 hover:text-indigo-600 dark:hover:text-indigo-400 transition-transform duration-200 shadow-2xs shrink-0 cursor-pointer"
                                    ...
                                    {{-- Phase Number Badge --}}
                                    <span class="w-7 h-7 rounded-xl flex items-center justify-center font-black text-xs text-white shadow-xs shrink-0 cursor-pointer hover:scale-105 active:scale-95 transition-transform"
                                    ...
                                    <div class="min-w-0 flex-1 flex flex-col sm:flex-row sm:items-center gap-2">
                                        <input type="text" x-model="phase.name" required
                                               ...
                                               class="font-black text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-gray-500 text-sm sm:text-base bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 px-3.5 py-2 focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 flex-1">

                                        <input type="text" x-model="phase.description"
                                               placeholder="توضیحات فاز (اختیاری)..."
                                               class="text-xs text-gray-500 dark:text-gray-400 placeholder-gray-400 dark:placeholder-gray-500 bg-white/80 dark:bg-gray-800/80 rounded-xl border border-gray-200/80 dark:border-gray-700/60 px-3 py-2 focus:ring-1 focus:ring-indigo-500 sm:w-1/3">
                                    </div>

                                    {{-- Color Picker & Presets --}}
                                    <div class="flex items-center gap-1.5 shrink-0" title="انتخاب رنگ فاز">
                                        <div class="relative flex items-center justify-center w-7 h-7 rounded-xl cursor-pointer shadow-2xs border border-black/10 overflow-hidden"
                                             :style="'background-color: ' + (phase.color || '#6366f1')">
                                ...
                                <div class="flex items-center gap-2 shrink-0">
                                    ...
                                    <button type="button" @click="addTaskToPhase(pIdx)"
                                            class="px-3.5 py-2 rounded-xl bg-white dark:bg-gray-800 hover:bg-indigo-50 hover:text-indigo-600 dark:hover:bg-indigo-900/40 text-gray-700 dark:text-gray-300 text-xs font-bold border border-gray-200 dark:border-gray-700 transition-all flex items-center gap-1.5 shadow-2xs cursor-pointer">
                            ...
                            <div x-show="!collapsedPhases[pIdx]"
                                ...
                                <template x-if="!phase.tasks || phase.tasks.length === 0">
                                    <div class="py-8 text-center text-gray-400 text-xs bg-white/60 dark:bg-gray-800/50 rounded-2xl border border-dashed border-gray-200 dark:border-gray-700">
                                ...
                                <template x-for="(task, tIdx) in phase.tasks" :key="'p-' + pIdx + '-t-' + tIdx">
                                    <div class="bg-white dark:bg-gray-800 rounded-2xl border transition-all p-4 sm:p-5 space-y-4 shadow-2xs"
                                        ...
                                        <div class="flex flex-col gap-3">
                                                ...
                                                <div class="flex items-start sm:items-center gap-3 min-w-0 flex-1">
                                                    ...
                                                    <label class="flex items-center gap-2 cursor-pointer p-1.5 rounded-xl hover:bg-indigo-50 dark:hover:bg-indigo-900/30 transition-colors shrink-0"
                                                        ...
                                                        <input type="checkbox"
                                                               ...
                                                               class="w-4 h-4 rounded-md text-indigo-600 focus:ring-2 focus:ring-indigo-500/20 border-gray-300 dark:border-gray-600 dark:bg-gray-700/80 transition-all cursor-pointer">
                                                        <span class="text-xs font-bold text-gray-500 dark:text-gray-400 select-none hidden lg:inline">انتخاب</span>
                                                    </label>

                                                    {{-- Collapse / Expand Chevron Icon --}}
                                                    <button type="button" @click="toggleTask('p' + pIdx + '_t' + tIdx)"
                                                            class="p-1.5 rounded-xl bg-gray-50 dark:bg-gray-700/60 border border-gray-200 dark:border-gray-700 text-gray-500 transition-transform duration-200 shadow-2xs shrink-0 cursor-pointer"
                                                    ...
                                                    <span class="w-8 h-8 rounded-xl flex items-center justify-center shrink-0 shadow-2xs cursor-pointer hover:scale-105 active:scale-95 transition-transform"
                                                    ...
                                                    <div class="min-w-0 flex-1 space-y-1.5">
                                                        <input type="text" x-model="task.title" required
                                                               ...
                                                               class="w-full font-bold text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-gray-500 text-sm sm:text-base rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-900/50 px-3.5 py-1.5 focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all">
                                                        <input type="text" x-model="task.description"
                                                               placeholder="توضیحات تکمیلی این گروه (اختیاری)..."
                                                               class="w-full text-xs text-gray-500 dark:text-gray-400 placeholder-gray-400 dark:placeholder-gray-500 rounded-lg border border-gray-100 dark:border-gray-700/60 bg-gray-50/30 dark:bg-gray-900/30 px-3 py-1 focus:ring-1 focus:ring-indigo-500 transition-all">
                                                    </div>
                                                </div>

                                                {{-- Group Actions --}}
                                                <div class="flex items-center gap-2 flex-wrap shrink-0">
                                                    <span class="inline-flex items-center gap-1 px-3 py-1 rounded-xl text-xs font-bold bg-emerald-50 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-300 border border-emerald-100 dark:border-emerald-800/40 shadow-2xs"
                                            ...
                                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-3 border-t border-gray-100 dark:border-gray-700/50">
                                                {{-- Manager Select --}}
                                                <div class="flex items-center gap-2">
                                                    <label class="text-xs font-bold text-gray-600 dark:text-gray-400 shrink-0 flex items-center gap-1.5">
                                                    ...
                                                    <select x-model="task.manager_id"
                                                        ...
                                                        <option value="" class="bg-white dark:bg-gray-800 text-gray-900 dark:text-white">(بدون مدیر گروه)</option>
                                                        @foreach($users as $u)
                                                            <option value="{{ $u->id }}" class="bg-white dark:bg-gray-800 text-gray-900 dark:text-white">{{ $u->name }}</option>
                                                        @endforeach
                                                    </select>
                                                </div>
                                                ...
                                                <div class="flex items-center gap-2">
                                                    ...
                                                    <div class="relative flex items-center w-full">
                                                        <input type="text" data-jdp data-jdp-only-date
                                                               ...
                                                               class="w-full rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900/50 px-3 py-1.5 text-xs font-medium text-gray-800 dark:text-gray-200 placeholder-gray-400 dark:placeholder-gray-500 focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 cursor-pointer">
                                        ...
                                        <div x-show="!collapsedTasks['p' + pIdx + '_t' + tIdx]"
                                            ...
                                            <div class="space-y-2.5">
                                                <template x-if="!task.items || task.items.length === 0">
                                                    <div class="py-3 text-center text-xs text-gray-400 bg-gray-50/50 dark:bg-gray-900/30 rounded-xl border border-dashed border-gray-200 dark:border-gray-700">
                                                        هنوز کاری برای این گروه اضافه نشده است. روی «+ کار جدید» کلیک کنید.
                                                    </div>
                                                </template>

                                                <template x-for="(item, iIdx) in task.items" :key="'p-' + pIdx + '-t-' + tIdx + '-i-' + iIdx">
                                                    <div class="group flex flex-col md:flex-row md:items-center justify-between gap-2.5 p-3 rounded-2xl bg-gray-50/70 dark:bg-gray-900/40 border border-gray-200/70 dark:border-gray-700/60 hover:border-indigo-300 dark:hover:border-indigo-700 transition-all shadow-2xs">
                                                        {{-- Work Item Title & Description --}}
                                                        <div class="flex items-center gap-2.5 min-w-0 flex-1">
                                                            <span class="w-6 h-6 rounded-lg bg-indigo-50 dark:bg-indigo-900/40 text-indigo-600 dark:text-indigo-400 text-[10px] font-black flex items-center justify-center shrink-0"
                                                                  x-text="iIdx + 1"></span>

                                                            <input type="text" x-model="item.title" required
                                                                   placeholder="عنوان کار یا فعالیت اجرایی..."
                                                                   class="flex-1 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 px-3 py-1.5 text-xs font-bold text-gray-800 dark:text-gray-200 placeholder-gray-400 dark:placeholder-gray-500 focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all">

                                                            <input type="text" x-model="item.description"
                                                                   placeholder="توضیحات تکمیلی کار..."
                                                                   class="w-1/3 sm:w-2/5 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 px-3 py-1.5 text-xs text-gray-500 dark:text-gray-400 placeholder-gray-400 dark:placeholder-gray-500 focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all">
                                                        </div>
                                                        ...
                                                        <div class="flex items-center gap-2 shrink-0 flex-wrap sm:flex-nowrap">
                                                            ...
                                                            <div class="flex items-center gap-1.5">
                                                                <select x-model="item.assigned_to"
                                                                    ...
                                                                    <option value="" class="bg-white dark:bg-gray-800 text-gray-900 dark:text-white">(پیش‌فرض مدیر گروه)</option>
                                                                    @foreach($users as $u)
                                                                        <option value="{{ $u->id }}" class="bg-white dark:bg-gray-800 text-gray-900 dark:text-white">{{ $u->name }}</option>
                                                                    @endforeach
                                                                </select>
                                                            ...
                                                            <div class="relative flex items-center w-28 sm:w-32">
                                                                <input type="text" data-jdp data-jdp-only-date
                                                                       ...
                                                                       class="w-full rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 px-2.5 py-1.5 text-xs text-gray-700 dark:text-gray-300 placeholder-gray-400 dark:placeholder-gray-500 focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 cursor-pointer">
                                                            ...
                                                            {{-- Delete Item Action --}}
                                                            <button type="button" @click="removeItemFromPhaseTask(pIdx, tIdx, iIdx)"
                ...
                {{-- ======================================================== --}}
                {{-- PARTITION 3-B: گروه‌های عمومی و بدون فاز (Unphased Tasks) --}}
                {{-- ======================================================== --}}
                <div class="bg-white dark:bg-gray-800/90 rounded-3xl border border-gray-200/90 dark:border-gray-700/70 shadow-xs overflow-hidden"
                     style="border-top: 5px solid #0284c7">

                    {{-- Unphased Header Bar --}}
                    <div class="p-4 sm:p-5 bg-sky-50/50 dark:bg-sky-950/30 border-b border-gray-200/80 dark:border-gray-700/60 flex items-center justify-between gap-4 select-none flex-wrap">
                        <div class="flex items-center gap-3 min-w-0 flex-1">
                            <button type="button" @click="togglePhase('unphased')"
                                    class="p-2 rounded-xl bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-gray-500 dark:text-gray-400 hover:text-sky-600 dark:hover:text-sky-400 transition-transform duration-200 shadow-2xs shrink-0 cursor-pointer"
                            ...
                            <span class="w-8 h-8 rounded-xl bg-sky-100 dark:bg-sky-900/40 text-sky-700 dark:text-sky-300 flex items-center justify-center font-bold text-xs shrink-0 shadow-2xs">
                                <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10"/>
                                </svg>
                            </span>
                            ...
                            <div>
                                ...
                                <p class="text-[11px] text-gray-400 mt-0.5">
                                    کارهایی که وابستگی زمانی یا مرحله‌ای به فاز مشخصی ندارند و در طول کل پروژه جاری هستند.
                                </p>
                            </div>
                        </div>

                        <div class="flex items-center gap-2 shrink-0">
                            <span class="px-2.5 py-1 rounded-xl text-xs font-bold bg-sky-50 text-sky-700 dark:bg-sky-950/40 dark:text-sky-300 border border-sky-200/60 dark:border-sky-800/40 shadow-2xs"
                    ...
                    <div x-show="!collapsedPhases['unphased']"
                        ...
                        <template x-if="!structure.unphased_tasks || structure.unphased_tasks.length === 0">
                            <div class="py-6 text-center text-gray-400 text-xs bg-white/60 dark:bg-gray-800/50 rounded-2xl border border-dashed border-gray-200 dark:border-gray-700">
                                گروه عمومی ثبت نشده است. در صورت تمایل می‌توانید با کلیک بر روی «+ افزودن گروه عمومی» اضافه کنید.
                            </div>
                        </template>
                                ...
                                <div class="flex flex-col gap-3">
                                    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 select-none">
                                        <div class="flex items-start sm:items-center gap-3 min-w-0 flex-1">
                                            ...
                                            <span class="w-8 h-8 rounded-xl bg-sky-50 dark:bg-sky-900/30 text-sky-600 dark:text-sky-400 flex items-center justify-center shrink-0 border border-sky-200 dark:border-sky-800/40 cursor-pointer hover:scale-105 active:scale-95 transition-transform"
                                ...
                                <div x-show="!collapsedTasks['unphased_t' + tIdx]"
                                    ...
                                    <div class="space-y-2.5">
                                        ...
                                        <template x-for="(item, iIdx) in task.items" :key="'unphased-t-' + tIdx + '-i-' + iIdx">
                                            <div class="group flex flex-col md:flex-row md:items-center justify-between gap-2.5 p-3 rounded-2xl bg-gray-50/70 dark:bg-gray-900/40 border border-gray-200/70 dark:border-gray-700/60 hover:border-indigo-300 dark:hover:border-indigo-700 transition-all shadow-2xs">
                                                <div class="flex items-center gap-2.5 min-w-0 flex-1">
                                                    <span class="w-6 h-6 rounded-lg bg-sky-50 dark:bg-sky-900/40 text-sky-600 dark:text-sky-400 text-[10px] font-black flex items-center justify-center shrink-0"
                                                          x-text="iIdx + 1"></span>
                                                ...
                                                <div class="flex items-center gap-2 shrink-0 flex-wrap sm:flex-nowrap">
                                                    ...
                                                    <button type="button" @click="removeItemFromUnphasedTask(tIdx, iIdx)"
                                                            class="p-1.5 rounded-lg text-gray-400 hover:text-rose-600 hover:bg-rose-50 dark:hover:bg-rose-900/30 dark:hover:text-rose-400 transition-colors cursor-pointer"
                                                            title="حذف کار">
                                                        <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                                            <path stroke-linecap="round" stroke-linejoin="round" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/>
            ...
            {{-- ======================================================== --}}
            {{-- PARTITION 4: نوار ابزار چسبان آمار و ذخیره نهایی         --}}
            {{-- ======================================================== --}}
            <div class="sticky bottom-4 z-30 bg-white/95 dark:bg-gray-800/95 backdrop-blur-xl rounded-3xl p-4 sm:p-5 border border-gray-200/80 dark:border-gray-700/80 shadow-xl flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                ...
                <div class="flex items-center gap-3 flex-wrap">
                    <div class="flex items-center gap-2 text-xs font-bold text-gray-600 dark:text-gray-300">
                        ...
                        <span class="text-indigo-600 dark:text-indigo-400 font-black" x-text="(structure.phases ? structure.phases.length : 0) + ' فاز'"></span>
                        <span>•</span>
                        <span class="text-indigo-600 dark:text-indigo-400 font-black" x-text="totalTasksCount + ' گروه'"></span>
                        <span>•</span>
                        <span class="text-indigo-600 dark:text-indigo-400 font-black" x-text="totalItemsCount + ' کار'"></span>
                    </div>
                    ...
                    <div x-show="totalSelectedCount > 0" x-cloak x-transition
                        ...
                        <span class="text-xs font-bold text-indigo-700 dark:text-indigo-300 bg-indigo-50 dark:bg-indigo-900/40 px-2.5 py-1 rounded-xl"
                ...
                <div class="flex items-center gap-2.5 justify-end">
                    ...
                    <button type="submit"
                        ...
                        <span>ذخیره نهایی تغییرات الگو</span>
                    </button>
                </div>
            </div>
        </form>
                                ...
                                tasks: [
                                    {
                                        ...
                                        due_date: "1404/02/10",
                                        items: [
                                            { title: "وایرفریمینگ ساختار کلی", description: "", assigned_to: "", due_date: "1404/01/28" },
                                            { title: "طراحی دیزاین سیستم و کامپوننت‌ها", description: "", assigned_to: "", due_date: "1404/02/05" },
                                            { title: "تأیید نهایی طرح از سمت کارفرما", description: "", assigned_to: "", due_date: "1404/02/10" }
                                        ]
                                    }
                                ]
                            }
