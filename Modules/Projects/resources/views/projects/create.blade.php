...
@php
    $todayJalali = function_exists('jdate')
        ? jdate()->format('Y/m/d')
        : (class_exists(\Morilog\Jalali\Jalalian::class) ? \Morilog\Jalali\Jalalian::now()->format('Y/m/d') : now()->format('Y/m/d'));
    ...
    if ($oldMembers && is_array($oldMembers)) {
        ...
        $initialMembers = [
            [
                ...
                'user_email' => $currentUser->email ?? ($currentUser->mobile ?? ''),
                'role'       => $defaultCreatorRole ?? 'manager',
            ]
        ];
    }
@endphp
...
@section('content')
    <div class="max-w-screen-2xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6"
         x-data="projectForm({
             ...
             startDate: '{{ old('start_date', $todayJalali) }}',
        ...
        {{-- Create Form --}}
        <form method="POST" action="{{ route('projects.projects.store') }}" class="space-y-6">
            @csrf
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-1.5">
                        <label class="block text-xs font-bold text-gray-700 dark:text-gray-300">
                            مشتری مربوطه @if(!empty($requireClient))
                    ...
                    <div>
                        ...
                        <select name="category_id" required
                            ...
                            @foreach($categories as $category)
                                <option
                                    value="{{ $category->id }}" @selected(old('category_id', $defaultCategoryId ?? null) == $category->id)>
                                    {{ $category->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select name="status_id"
                            ...
                            @foreach($statuses as $status)
                                <option
                                    value="{{ $status->id }}" @selected(old('status_id', $defaultStatusId ?? ($status->is_default ? $status->id : null)) == $status->id)>
                                    {{ $status->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        <div class="flex items-center justify-between mb-2">
                            ...
                            @if(!empty($codeAuto))
                                <span
                                    class="text-[10px] px-2 py-0.5 rounded bg-indigo-50 text-indigo-600 dark:bg-indigo-900/30 dark:text-indigo-400 font-bold">شماره‌گذاری خودکار فعال</span>
                            @endif
                        </div>
                    ...
                    <div>
                        <label class="block text-xs font-bold text-gray-700 dark:text-gray-300 mb-2">
                            تاریخ شروع @if(!empty($requireDates))
                    ...
                    <div>
                        <label class="block text-xs font-bold text-gray-700 dark:text-gray-300 mb-2">
                            تاریخ پایان / سررسید @if(!empty($requireDates))
                                <span class="text-rose-500 font-black">*</span>
                            @endif
                        </label>
                        <div class="relative">
                                ...
                                <div class="w-full sm:w-48 shrink-0">
                                    ...
                                    <select :name="`members[${index}][role]`" x-model="member.role"
                                        ...
                                        @if(isset($roles) && $roles->isNotEmpty())
                                            @foreach($roles as $r)
                                                <option value="{{ $r->name }}">{{ $r->display_name }}
                                                    ({{ ucfirst($r->name) }})
                                                </option>
                                            @endforeach
                                        @else
            ...
            <div class="sticky bottom-4 z-40">
                <div
                    ...
                    <button type="submit"
                            class="flex-1 md:flex-none px-8 py-3.5 rounded-xl bg-linear-to-r from-indigo-600 to-indigo-700 text-white font-black shadow-lg shadow-indigo-500/30 hover:shadow-indigo-500/50 transition-all duration-300 active:scale-95 flex items-center justify-center gap-2">
            ...
            function projectForm(config = {}) {
                return {
                    members: (config.initialMembers || []).map((m, idx) => ({
                        ...
                        role: m.role || '{{ $defaultMemberRole ?? "viewer" }}',
                        searchQuery: '',
                        dropdownOpen: false,
                        searching: false,
                        searchResults: [],
