    ...
    <div class="max-w-screen-2xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8"
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <h1 class="text-2xl font-black text-gray-900 dark:text-white flex items-center gap-4 tracking-tight">
                <span
                    class="flex items-center justify-center w-12 h-12 rounded-2xl bg-linear-to-br from-amber-500 to-amber-700 text-white shadow-lg shadow-amber-500/30">
        ...
        <form method="POST" action="{{ route('projects.categories.update', $category) }}" class="space-y-8"
            ...
            <div class="{{ $cardClass }}">
                ...
                <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="md:col-span-2">
                        ...
                        <input type="text" name="name" x-model="name" value="{{ old('name', $category->name) }}"
                               required
                               class="{{ $inputClass }}" placeholder="مثال: طراحی و پیاده‌سازی وب">
                    </div>

                    <div>
                        <label class="{{ $labelClass }}">رنگ سازمانی</label>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <div class="flex items-center gap-4">
                            <div
                                class="w-16 h-16 rounded-2xl border-2 border-dashed border-gray-200 dark:border-gray-700 flex items-center justify-center overflow-hidden bg-gray-50 dark:bg-gray-900/40 shrink-0">
                                <img x-show="preview && !removeIcon" :src="preview" class="w-full h-full object-cover"
                                     alt="">
                                <svg x-show="!preview || removeIcon" class="w-6 h-6 text-gray-300 dark:text-gray-600"
                            ...
                            <div class="flex-1 space-y-2.5">
                                ...
                                @if($category->icon)
                                    <label
                                        class="flex items-center gap-2 text-xs font-bold text-red-500 dark:text-red-400 cursor-pointer w-fit">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <select name="status" class="{{ $inputClass }} cursor-pointer md:max-w-xs">
                            <option value="active" @selected(old('status', $category->status) === 'active')>فعال
                            </option>
            ...
            @if($category->projects()->count() > 0)
                <div
                    class="rounded-2xl bg-amber-50 dark:bg-amber-500/10 p-5 border border-amber-200 dark:border-amber-500/20 text-sm text-amber-800 dark:text-amber-300 flex items-start gap-4">
                    ...
                    <div>
                        <span class="font-bold block mb-1">عدم امکان حذف دسته‌بندی:</span>
                        <span>این دسته‌بندی به {{ $category->projects()->count() }} پروژه متصل است و تا زمان حذف یا انتقال این پروژه‌ها، امکان حذف این دسته‌بندی وجود ندارد. تغییر وضعیت به «غیرفعال» پروژه‌های متصل را حذف نخواهد کرد.</span>
                    </div>
                </div>
            @endif
