<div
    ...
    x-show="isOpen"
    x-on:keydown.escape.window="if (!$wire.showNewAddressModal) isOpen = false"
...
>
    ...
    <div class="flex items-end justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div x-show="isOpen"
             ...
             aria-hidden="true"
             @click="if (!$wire.showNewAddressModal) isOpen = false">
        </div>
        ...
        <div x-show="isOpen"
             ...
             @click.outside="if (!$wire.showNewAddressModal) isOpen = false">
                                        ...
                                        @if(Auth::guard('client')->check() && $isAddressField)
                                            @if(!$addressSelectorRendered)
                                                ...
                                                <div class="sm:col-span-2 space-y-4" wire:key="modal-address-selector-block-{{ $group['id'] }}">
                                                    <div class="bg-gray-50/50 dark:bg-gray-850/20 border border-gray-200 dark:border-gray-700/60 rounded-2xl p-5 space-y-4">
                                                            ...
                                                            <div class="relative bg-white dark:bg-gray-800/60 border border-indigo-500/20 rounded-xl p-4 shadow-sm">
                                                                <div class="flex items-center justify-between mb-2">
                                                                    ...
                                                                    <div x-data="{ open: false }" class="relative">
                                                                        ...
                                                                        <div x-show="open" @click.away="open = false" x-transition class="absolute left-0 mt-2 w-64 bg-white dark:bg-gray-850 border border-gray-200 dark:border-gray-700 rounded-xl shadow-xl z-50 py-1.5 overflow-hidden" style="display: none;">
                                                                            @foreach($addresses as $addr)
                                                                                <button type="button" @click="open = false" wire:click="selectAddress({{ $addr->id }})" class="w-full text-right px-4 py-2.5 text-xs hover:bg-gray-50 dark:hover:bg-gray-750 flex flex-col gap-1 border-b border-gray-100 dark:border-gray-700 last:border-0 transition-colors">
                                ...
                                <div
                                    ...
                                    class="relative flex items-center p-4 border rounded-xl cursor-pointer transition-all duration-300
                                        {{ $payment_method === $key
                                            ...
                                            : 'bg-gray-55/50 dark:bg-gray-800/30 border-gray-200 dark:border-gray-700 hover:border-gray-400 dark:hover:border-gray-600'
                                        }}"
                                >
    ...
    @if($showNewAddressModal)
        <div wire:key="modal-new-address-modal" class="fixed inset-0 z-[200] flex items-center justify-center overflow-x-hidden overflow-y-auto outline-none focus:outline-none" style="backdrop-filter: blur(8px); background-color: rgba(0, 0, 0, 0.4);">
