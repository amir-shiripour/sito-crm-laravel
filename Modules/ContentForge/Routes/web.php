...
Route::middleware(['web'])->group(function() {
    ...
    Route::match(['get', 'post'], '/{slug}', [ContentFrontController::class, 'showDefault'])
         ->where('slug', '^(?!(user|admin|api|login|logout|register|storage|js|css|fonts|images|sitemap\.xml|feed\.xml|blog|s/|livewire|settings|booking|shop|cart|checkout|market|install|ping|chat)).+')
