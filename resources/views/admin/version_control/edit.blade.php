            ...
            <nav class="flex mb-8" aria-label="Breadcrumb">
                <ol class="inline-flex items-center space-x-3 space-x-reverse">
                    ...
                    <li aria-current="page">
                        <div class="flex items-center">
                            <svg class="w-3 h-3 text-gray-400 mx-1" fill="currentColor" viewBox="0 0 20 20"><path d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z"/></svg>
                            <span class="mr-1 text-sm font-bold text-indigo-600 dark:text-indigo-400">ویرایش نسخه</span>
                        </div>
                    </li>
                </ol>
            </nav>

            <form action="{{ route('admin.version-control.update', $version) }}" method="POST" class="relative">
                @csrf
                @method('PUT')
                ...
                <div class="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div>
                        <div class="flex items-center gap-3 mb-2">
                            <h1 class="text-3xl font-black text-slate-900 dark:text-white tracking-tight">
                                ویرایش <span class="text-transparent bg-clip-text bg-gradient-to-r from-amber-500 to-orange-600">اطلاعات نسخه</span>
                            </h1>
                            <div class="px-3 py-1 bg-indigo-50 dark:bg-indigo-500/10 border border-indigo-100 dark:border-indigo-500/20 text-indigo-600 dark:text-indigo-400 rounded-xl text-xs font-mono font-bold tracking-wider">
                                v{{ $version->version_number }}
                            </div>
                        </div>
                        <p class="text-slate-500 dark:text-slate-400 font-medium text-sm">تغییرات ثبت شده و جزئیات ورژن فعلی را اصلاح کنید.</p>
                    </div>
                    <div class="flex items-center gap-3">
                        ...
                        <button type="submit" class="group relative inline-flex items-center justify-center px-8 py-3 font-bold text-white transition-all duration-200 bg-indigo-600 font-pj rounded-2xl focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-600 shadow-xl shadow-indigo-600/20 hover:bg-indigo-700 active:scale-95">
                            بروزرسانی نهایی
                        </button>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                                ...
                                <div class="space-y-2">
                                    ...
                                    <div class="relative group">
                                        ...
                                        <input type="text" name="version_number" id="version_number" value="{{ old('version_number', $version->version_number) }}" required
                                ...
                                <div class="space-y-2">
                                    ...
                                    <div class="relative group">
                                        ...
                                        <input type="date" name="release_date" id="release_date" value="{{ old('release_date', $version->release_date->format('Y-m-d')) }}" required
                                ...
                                <div class="md:col-span-2 space-y-2">
                                    <x-label for="title" value="عنوان اختصاصی نسخه" class="text-sm font-bold text-slate-700 dark:text-slate-300 mr-1" />
                                    <input type="text" name="title" id="title" value="{{ old('title', $version->title) }}"
                                           ...
                                           placeholder="مثلاً: بهبود عملکرد هسته مرکزی">
                                    <x-input-error for="title" />
                                </div>

                                {{-- Summary --}}
                                <div class="md:col-span-2 space-y-2">
                                    <x-label for="summary" value="توضیحات و خلاصه تغییرات" class="text-sm font-bold text-slate-700 dark:text-slate-300 mr-1" />
                                    <textarea name="summary" id="summary" rows="4"
                                              ...
                                              placeholder="شرح مختصری از اهداف این ورژن...">{{ old('summary', $version->summary) }}</textarea>
                                    <x-input-error for="summary" />
                                </div>
                            </div>
                        </div>

                        {{-- Changelog Dynamic List --}}
                        <div x-data="{ items: {{ json_encode(old('changelog', $version->changelog ?? [''])) }} }" class="bg-white/70 dark:bg-slate-900/50 backdrop-blur-xl border border-white dark:border-slate-800 shadow-2xl shadow-slate-200/50 dark:shadow-none rounded-[2.5rem] p-8 transition-all">
                            <div class="flex items-center justify-between mb-8">
                                <div>
                                    <h3 class="text-lg font-black text-slate-900 dark:text-white">لیست جزئیات تغییرات (Changelog)</h3>
                                    <p class="text-xs text-slate-500 dark:text-slate-400 mt-1">تغییرات فنی یا قابلیت‌های جدید را به صورت جداگانه وارد کنید.</p>
                                </div>
                                <button type="button" @click="items.push('')" class="flex items-center gap-2 px-4 py-2 bg-indigo-50 dark:bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 rounded-xl text-xs font-bold hover:bg-indigo-100 dark:hover:bg-indigo-500/20 transition-all active:scale-95">
                            ...
                            <div class="space-y-4">
                                <template x-for="(item, index) in items" :key="index">
                                    <div class="flex items-center gap-4 group animate-in slide-in-from-right duration-300">
                                        <span class="flex-shrink-0 w-10 h-10 rounded-2xl bg-amber-50 dark:bg-amber-500/10 text-amber-600 dark:text-amber-400 flex items-center justify-center text-xs font-mono font-bold border border-amber-100 dark:border-amber-500/20 shadow-sm" x-text="index + 1"></span>
                                        <div class="flex-1">
                                        ...
                                        <button type="button" @click="items.splice(index, 1)" class="p-3 text-slate-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-500/10 rounded-2xl transition-all opacity-0 group-hover:opacity-100 focus:opacity-100">
                            ...
                            <div x-show="items.length === 0" class="py-12 border-2 border-dashed border-slate-200 dark:border-slate-800 rounded-[2rem] text-center">
                                <p class="text-sm text-slate-400 dark:text-slate-500 italic font-medium">هیچ تغییری یافت نشد. با استفاده از دکمه بالا تغییرات را اضافه کنید.</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="lg:col-span-1 space-y-8">
                        ...
                        <div class="bg-white/70 dark:bg-slate-900/50 backdrop-blur-xl border border-white dark:border-slate-800 shadow-2xl shadow-slate-200/50 dark:shadow-none rounded-[2.5rem] p-8">
                            ...
                            <div class="p-6 bg-indigo-50 dark:bg-indigo-500/5 rounded-[2rem] border border-indigo-100 dark:border-indigo-500/10">
                                <div class="flex items-start gap-4">
                                    <div class="mt-1">
                                        <input type="checkbox" name="is_current" id="is_current" value="1" {{ old('is_current', $version->is_current) ? 'checked' : '' }}
                                        class="w-6 h-6 text-indigo-600 bg-white dark:bg-slate-800 border-slate-300 dark:border-slate-700 rounded-lg focus:ring-indigo-500 dark:focus:ring-offset-slate-900 transition-all cursor-pointer">
                                    </div>
                                    <div>
                                        ...
                                        <p class="mt-2 text-[11px] text-slate-500 dark:text-slate-400 leading-relaxed">
                                            با فعال کردن این گزینه، این شماره ورژن در سایدبار پنل تمام کاربران نمایش داده می‌شود.
                                        </p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="mt-8 pt-8 border-t border-slate-100 dark:border-slate-800">
                                <h4 class="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-4">آمار نسخه</h4>
                                <div class="space-y-4">
                                    <div class="flex justify-between items-center text-xs font-medium">
                                        <span class="text-slate-500 dark:text-slate-400">تاریخ ثبت اولیه:</span>
                                        <span class="text-slate-900 dark:text-white font-mono">{{ $version->created_at->format('Y/m/d') }}</span>
                                    </div>
                                    <div class="flex justify-between items-center text-xs font-medium">
                                        <span class="text-slate-500 dark:text-slate-400">آخرین بروزرسانی:</span>
                                        <span class="text-slate-900 dark:text-white font-mono">{{ $version->updated_at->format('Y/m/d H:i') }}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        {{-- Pro Tip Card --}}
                        <div class="bg-gradient-to-br from-amber-500 to-orange-600 rounded-[2.5rem] p-8 text-white shadow-2xl shadow-orange-500/30 overflow-hidden relative group">
                            <div class="absolute -right-4 -bottom-4 opacity-10 group-hover:scale-110 transition-transform duration-700">
                                <svg class="w-40 h-40" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2a10 10 0 1010 10A10 10 0 0012 2zm1 15h-2v-6h2zm0-8h-2V7h2z" /></svg>
                            </div>
                            <h4 class="text-lg font-bold mb-2">راهنمای ویرایش</h4>
                            <p class="text-xs text-amber-50 leading-relaxed opacity-80 font-medium">
                                تغییر شماره نسخه ممکن است بر سیستم‌های کش یا بررسی آپدیت ماژول‌ها تأثیر بگذارد. قبل از تغییرات گسترده از صحت داده‌ها اطمینان حاصل کنید.
                            </p>
                        </div>
                    </div>
                </div>
