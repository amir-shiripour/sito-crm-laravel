...
@php
    $title = 'ویرایش نقش: ' . ($role->display_name ?? $role->name);

    // 🚨 هشدار معماری: این خطوط باید در کنترلر هندل شوند.
    $widgetGroups = collect($widgets ?? [])->groupBy('group');
    $oldWidgets = old('widgets');
    $isSuperAdmin = in_array($role->name, ['super-admin', 'Super Admin']);
    ...
    <div class="max-w-5xl mx-auto px-4 py-8 space-y-6">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
            <div>
                ...
                <span class="flex items-center justify-center w-8 h-8 rounded-lg bg-amber-100 text-amber-600 dark:bg-amber-500/20 dark:text-amber-300">
                    <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg>
                </span>
                    ویرایش نقش
                </h1>
                <p class="text-sm text-gray-500 dark:text-gray-400 mt-1 mr-10">
                    ویرایش اطلاعات، ویجت‌ها و دسترسی‌های نقش <span class="font-bold text-gray-700 dark:text-gray-300">{{ $role->display_name ?? $role->name }}</span>
                </p>
            </div>
                ...
                @if(!$isSuperAdmin)
                    @can('roles.delete')
                        <form method="POST" action="{{ route('admin.roles.destroy', $role) }}" class="inline-block" onsubmit="return confirm('آیا از حذف این نقش اطمینان دارید؟')">
                            @csrf @method('DELETE')
                            <button type="submit" class="inline-flex items-center gap-2 px-4 py-2 text-sm font-bold rounded-xl bg-red-50 text-red-600 hover:bg-red-100 dark:bg-red-900/20 dark:text-red-400 dark:hover:bg-red-900/40 transition-colors border border-transparent dark:border-red-800">
        ...
        <form method="POST" action="{{ route('admin.roles.update', $role) }}" class="space-y-6 pb-20" x-data="{ isSubmitting: false }" @submit="isSubmitting = true">
            @csrf @method('PUT')
            ...
            <div class="{{ $cardClass }}">
                ...
                <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text" name="display_name" value="{{ old('display_name', $role->display_name) }}" class="{{ $inputClass }}" required placeholder="مثلاً: مدیر فروش">
                        @error('display_name')
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <input type="text" name="name" value="{{ old('name', $role->name) }}"
                                   class="{{ $inputClass }} dir-ltr text-left font-mono {{ $isSuperAdmin ? 'opacity-70 cursor-not-allowed bg-gray-100 dark:bg-gray-800' : '' }}"
                        ...
                        @if($isSuperAdmin)
                            <p class="text-[10px] text-amber-600 dark:text-amber-500 mt-1.5 font-bold">نام لاتین نقش super-admin به دلایل امنیتی و سیستمی قابل تغییر نیست.</p>
                        @else
                        ...
                        <div class="bg-gray-50 dark:bg-gray-900/30 rounded-xl p-5 border border-gray-100 dark:border-gray-700/50">
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                                @foreach($groupWidgets as $widget)
                                    @php
                                        // 🚨 منطق پردازش oldValues
                                        if (is_array($oldWidgets)) {
                                            $checked = array_key_exists($widget['key'], $oldWidgets);
                                        } else {
                                            $checked = in_array($widget['key'], $roleWidgets ?? [], true);
                                        }
                                    @endphp
            ...
            <div class="flex items-center justify-between sticky bottom-6 z-40 bg-white/80 dark:bg-gray-900/80 backdrop-blur-md p-4 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-xl">
                ...
                <button type="submit"
                    ...
                    <span x-show="!isSubmitting">ذخیره تغییرات نقش</span>
