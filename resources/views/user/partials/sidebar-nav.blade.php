...
@if(!empty($isCustomEnabled) && !empty($menuBlocks))
    {{-- حالت منوی سفارشی: رندر دقیق بلوک‌ها بر اساس اولویت و چیدمان ذخیره‌شده --}}
    @foreach($menuBlocks as $block)
        @if($block['type'] === 'dashboard')
            ...
            <a href="{{ route('user.dashboard') }}"
               ...
               :title="sidebarCollapsed ? '{{ htmlspecialchars($dashItem['title'] ?? 'داشبورد', ENT_QUOTES) }}' : null">
                @if($isDashActive)
                ...
                <span x-show="!sidebarCollapsed" x-transition class="truncate">{{ $dashItem['title'] ?? 'پیشخوان' }}</span>
            </a>
        ...
        @elseif($block['type'] === 'group')
            @php
                ...
                $isGroupActive = request()->routeIs($block['module'] . '.*') || collect($block['items'])->contains(fn($i) => !empty($i['route']) && request()->routeIs($i['route'] . '*'));
                $groupTitle = $block['title'];
                $firstItem = $block['items'][0] ?? null;
            @endphp
            @if(count($validGroupItems) > 0)
            <div x-data="{ open: isMenuOpen('{{ $block['module'] }}') || {{ $isGroupActive ? 'true' : 'false' }} }" class="mt-1">
                <button @click="toggleMenu('{{ $block['module'] }}'); open = !open"
                    ...
                    <div class="flex items-center gap-3 overflow-hidden">
                        ...
                        <span x-show="!sidebarCollapsed" class="truncate font-semibold text-start">{{ $groupTitle }}</span>
                    </div>
        ...
        @elseif($block['type'] === 'settings')
            ...
            <div x-data="{ open: isMenuOpen('settings') || {{ $isSettingsActive ? 'true' : 'false' }} }" class="mt-1 pb-4">
                <button @click="toggleMenu('settings'); open = !open"
                        ...
                        :title="sidebarCollapsed ? '{{ htmlspecialchars($settingsTitle, ENT_QUOTES) }}' : null">
                    <div class="flex items-center gap-3 overflow-hidden">
                        <span class="{{ $iconClass }} [&>svg]:w-5 [&>svg]:h-5 {{ $isSettingsActive ? 'text-indigo-600 dark:text-indigo-400' : '' }}">
                        ...
                        <span x-show="!sidebarCollapsed" class="truncate font-semibold text-start">{{ $settingsTitle }}</span>
                    </div>
                    <svg x-show="!sidebarCollapsed" :class="open ? 'rotate-90 text-indigo-500' : '-rotate-90'" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="w-4 h-4 transition-transform duration-300 shrink-0 opacity-50 group-hover:opacity-100">
                        <path stroke="none" d="M0 0h24v24H0z" fill="none" /><path d="M15 6l-6 6l6 6" />
                    </svg>
...
@else
    {{-- حالت پیش‌فرض هسته سیستم: ۱۰۰٪ ساختار کلاسیک و دست‌نخورده --}}
    {{-- لینک پیشخوان --}}
    <a href="{{ route('user.dashboard') }}"
    ...
    @if(count($clientsItems) > 0)
        ...
        <div x-data="{ open: isMenuOpen('clients') || {{ $isClientsActive ? 'true' : 'false' }} }" class="mt-1">
            <button @click="toggleMenu('clients'); open = !open"
                    ...
                    :title="sidebarCollapsed ? '{{ htmlspecialchars($clientsTitle, ENT_QUOTES) }}' : null">

                <div class="flex items-center gap-3 overflow-hidden">
                    <span class="{{ $iconClass }} [&>svg]:w-5 [&>svg]:h-5 {{ $isClientsActive ? 'text-indigo-600 dark:text-indigo-400' : '' }}">
                    ...
                    <span x-show="!sidebarCollapsed" class="truncate font-semibold text-start">{{ $clientsTitle }}</span>
                </div>
    ...
    @foreach($menuGroups as $group)
        @php
            ...
            $groupTitle = !empty($group['title']) ? $group['title'] : (!empty($group['module_name']) ? $group['module_name'] : ($group['module'] === 'admin' ? __('menu.system_management') : __($group['module'] . '::menu.group_title')));
    ...
    @if(count($settingsItems) > 0)
        @php
            $isSettingsActive = request()->routeIs('settings.*') || request()->routeIs('user.settings.*');
            $settingsTitle = $settingsGroupMeta['title'] ?? 'تنظیمات';
